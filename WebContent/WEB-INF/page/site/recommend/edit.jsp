...
<script>
	$(document).ready(function(){
		$("#submitBtn").click(function(){
			$.post("/recommend/update.action", $("#form").serialize(), function(data){
...
<body>
	<form id="form">
		<input name="id" type="hidden" value="${recommend.id }"/>
		<div class="form_item">
			<span class="form_span">推荐类型名称：</span>
			<input name="name" type="text" value="${recommend.name }"/>
		</div>
		<!-- 
		<div class="form_item">
			...
			<select name="columnId">
				...
				<c:forEach items="${columnList }" var="column">
					<option value="${column.id }" <c:if test="${column.id == recommend.columnId }">selected="selected"</c:if>>
