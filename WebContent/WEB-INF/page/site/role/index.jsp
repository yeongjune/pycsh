...
<head>
	...
	function reloadRole(){
		$.post('/columnRole/load.action',{},function(data){
			static_roleId = 0;
	...
	function add(parentNode){
		if(siteId>0){
			...
			$.post('/columnRole/save.action',{name:'新角色', pid:pid, sort:0},function(data){
				if(data!='fail'){
			...
			art.dialog.alert('你不是网站管理员，不能添加栏目角色');
		}
	}
	...
	function updateRole(id,pid,name,index){
		pid = pid?pid:0;
		$.post('/columnRole/update.action',{id:id,name:name,pid:pid,sort:index},function(data){
			if(data=='succeed'){
	...
	function deleteRole(targetNode){
		...
		$.post('/columnRole/delete.action',{id:role_id},function(data){
			if(data!='succeed'){
				reloadRole();
				alert('删除失败');
			}else{
	...
	function zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {
		...
		function menuTreeOnCheck(event, treeId, treeNode) {
			if(static_roleId){
			    $.post('/roleColumn/update.action',{roleId:static_roleId, columnId:treeNode.id,checked:treeNode.checked?1:0},function(data){
			    	if(data=='succeed'){
		...
		function reloadMenu(roleId){
			if(roleId>0){
				$.post('/roleColumn/load.action',{roleId:roleId},function(data){
...
<body>
	<fieldset class="fieldset_border">
		<legend class="legend_title">栏目角色管理</legend>
