	...
	function ProjectAdd(){
		...
		this.save = function(){
			...
			if(""==name){
				...
				art.dialog.alert('筹款目标不能为空');
				return;
			}else if(""==startTime || ""==endTime){
				art.dialog.alert('筹款周期不能为空');
				return;
			}else if(""==introduce){
...
<body onunload="removeObject()">
	<form action="">
		...
		<div class="form_item">
			<span class="form_span"><font color="red" id="title_tip" >*</font>筹款目标(元)：</span>
		...
		<div class="form_item">
			...
			至
			<input type="text" name="endTime" id="endTime" class="piece" value="<fmt:formatDate value="${applyInfo.startTime }"  type="date" pattern="yyyy-MM-dd"/>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startTime\',{d:0})}'})" maxlength="10"  style="width:85px"  />
		</div>
