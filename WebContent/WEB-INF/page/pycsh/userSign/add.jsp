...
<script type="text/javascript">
	...
	function ProjectAdd(){
		...
		this.save = function(){
			...
			lockWindow();
			var id="${project.id}";
			var url = "/userSign/saveCharityProject.action";
			if(id!=""){
				var url = "/userSign/updateCharityProject.action";
			}
	...
	function toUploadApplication(){
		art.dialog.open('/userSign/toUploadPromise.action',{
			title:"申报书(doc,xls,docx,xlsx,pdf)",
			width:'auto',
			height:'auto',
			lock:true,
			close:function(){
				$("#application").val(this.iframe.contentWindow.$("#typeValue").val());
				$("#applicationSpan").html(this.iframe.contentWindow.$("#typeName").val());
				$("#originalName").val(this.iframe.contentWindow.$("#typeName").val());
			}
		});
...
<body onunload="removeObject()">
	<form action="">
		...
		<div class="form_item div_title_1_1">
			...
			<input type="text" id="name" class="piece" style="width:500px;" value="${project.name }" maxlength="100" />
		</div>
		<div class="form_item div_title_1_2">
			...
			<c:import url="/projectType/projectTypeSelect.action">
				<c:param name="id1" value="type"></c:param>
				<c:param name="defaultValue1" value="${project.type }"></c:param>
				<c:param name="category" value="game"></c:param>
			</c:import>
		</div>
		
		<div class="form_item">
			<span class="form_span"><font color="red" id="title_tip" >*</font>项目预算(元)：</span>
			<input type="text" id="targer" class="piece" style="width:500px;" value="<fmt:formatNumber value="${project.targer }" pattern="#.00"></fmt:formatNumber>"  onkeyup="if(isNaN(value))execCommand('undo')" onafterpaste="if(isNaN(value))execCommand('undo')"/>
		</div>
		<!-- 缩略图 -->
			...
			<table>
				<tr>
					...
					<td style="vertical-align: bottom;">
						<c:import url="/common/swfUpload8.jsp">
		...
		<div class="form_item div_title_1_1" style="margin-top: 10px;">
			...
			<a href="javascript:void(0);" class="base_btn" onclick="toUploadApplication();"><span>上传申报书</span></a>
			
			<span id="applicationSpan">${project.originalName }</span>
		</div>
		<!-- 文章内容结束 -->
		<div class="clear"></div>
