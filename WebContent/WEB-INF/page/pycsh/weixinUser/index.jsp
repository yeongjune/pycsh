...
<fieldset class="fieldset_border">
	<legend class="legend_title">用户管理</legend>
			<div class="form_search_item" >
				...
				--></div>
				<div class="right">
					<input type="text" class="piece" id="loginName" />
...
function ProjectManager(siteId){
	this.siteId = siteId;
	this.load = function(){
		loadByPage('/weixinUserManager/list.action',{loginName:$('#loginName').val()},this.showList);
	};
	this.showList = function(list){
		...
		for ( var item in list) {
			...
			if(project){
				...
				'	<td>'+(index+1)+'</td>'+
				'	<td><a href="javascript:projectManager.toView('+project.id+');">'+project.loginName+'</a></td>'+
				'	<td>'+(project.name==null?'':project.name)+'</td>'+
				'	<td>'+(project.idCard==null?'':project.idCard)+'</td>';
				if(project.gender==1){
					html += '<td>男</td>';
				}else{
					html += '<td>女</td>';
				...
				html += ''+
				'	<td>'+(project.email==null?'':project.email)+'</td>'+
				'	<td>'+(project.homehone==null?'':project.homehone)+'</td>'+
				'	<td>'+(project.liveAddress==null?'':project.liveAddress)+'</td>'+
				'	<td>'+project.time+'</td>';
				if(project.status==1){
					html += '<td><a href="javascript:projectManager.updateStatus('+project.id+',0);">禁用</a></td>';
				}else{
					html += '<td><a href="javascript:projectManager.updateStatus('+project.id+',1);">正常</a></td>';
				}
				html += '<td>';
	...
	this.toView = function(id){
		art.dialog.open('/weixinUserManager/toView.action?id='+id, {
			width: 600,
			height: 400,
			title:'用户详情'
		});
	...
	//修改用户的状态
	this.updateStatus = function(id,flag){
		$.post('/weixinUserManager/updateStatus.action',{id:id,flag:flag},function(data){
			if(data=='succeed'){
				projectManager.load();
				if(flag==0){
					art.dialog.tips('用户状态已改为正常');
				}else{
					art.dialog.tips('用户状态已改为禁用');
				}
			}else{
