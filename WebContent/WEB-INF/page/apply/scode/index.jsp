...
function MngScode(){
	this.load = function () {
		...
		loadByPage('/student/list.action',{status:$("#status").val(),examYear:$("#examYear").val(),name:$("#name").val(),interview:$("#interview").val(),admit:$("#admit").val()},this.showList);
	};
	...
	this.edit = function (id){
		art.dialog.open('/scode/toEdit.action?id='+id,{
			title : '个人成绩',
			width:'400px',
			height:'350px'
		});
    ...
    this.changeAdmit = function (admit){
		...
		if(admit==1){//录取
			art.dialog.confirm("您确定要"+temp+" [ "+names.join(",")+" ] 吗？", function(){
				$.post('/scode/saveAdmit.action',{ids:ids.join(','),admit:admit},function(data){
			...
			art.dialog.open('/student/toCheckRemark.action?admit=0&ids='+ids.join(',')+'&names='+names.join(','),{
				title:temp+'备注',
				width:'500px',
				height:'200px'
			});
    ...
    this.downScodeTemp = function (){
    	art.dialog.open('/scode/selectExam.action',{
   			title:'模版数据选项',
   			width:'300px',
   			ok:function(){
   		    	...
   		    	excelExport('interviewResult', 'scope_template', examId);
   		    	return true;
   			},
   			cancel:true
   		});
	...
	<legend class="legend_title">成绩管理</legend>
		...
		<div class="form_search_item">
			<div class="left">
				...
				<a href="javascript:mngScode.downScodeTemp()">下载成绩模版</a>
			</div>
			<div class="clear"></div>
		</div>
		<div id="scrollDiv">
