...
function MngScode(){
	this.load = function () {
		...
		loadByPage('/zd_student/list.action',{status:$("#status").val(),examYear:$("#examYear").val(),name:$("#name").val(),interview:$("#interview").val(),admit:$("#admit").val(),canInterviewDateNull:false},this.showList);
	};
	...
	this.edit = function (id){
		art.dialog.open('/mh_scode/toEdit.action?examType=1&id='+id,{
			title : '个人成绩',
			width:'400px',
			height:'350px'
		});
    ...
    this.changeAdmit = function (admit){
		...
		if(admit==-1){//待公布
			art.dialog.confirm("您确定要把 [ "+names.join(",")+" ]的录取状态设置成" + temp + " 吗？", function(){
				$.post('/mh_scode/saveAdmit.action',{ids:ids.join(','),admit:admit,examType:1},function(data){
			...
			art.dialog.open('/zd_student/toCheckRemark.action?examType=1&admit=' + admit + '&ids='+ids.join(',')+'&names='+names.join(','),{
				title:temp+'备注',
				width:'500px',
				height:'200px'
			});
	...
	<legend class="legend_title">成绩管理</legend>
		...
		<div class="form_search_item">
			<div class="left">
				...
				<a href="javascript:excelExport('interviewScoreMH', 'scope_template');" class="base_btn" ><span>导出</span></a>
			</div>
			<div class="clear"></div>
		</div>
		<div id="scrollDiv">
