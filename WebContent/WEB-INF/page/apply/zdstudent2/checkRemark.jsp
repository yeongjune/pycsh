...
function checkNotPass(){
	...
	var checkRemark;
	if($("#remarkType").val() == null){
		checkRemark= $("#remarkContent").val();
	}else{
		checkRemark=$("#remarkType").val()+(remarkContent?","+$("#remarkContent").val():'');
	}
	if(!$("#admit").val()){//修改报名状态
		var temp=$("#status").val()==2?"审核不通过":"审核回退";
		$.post('/zd_student2/doChangeStatus.action',{ids:$("#ids").val(),status:$("#status").val(),checkRemark:checkRemark},function(data){
		...
		var admit = $("#admit").val();
		$.post('/mh_scode/saveAdmit.action',{ids:$("#ids").val(),admit:admit,checkRemark:checkRemark,examType:'${examType}'},function(data){
			var content = '不录取操作成功';
			var error = '不录取操作失败';
			if(admit > 0){
				content = '录取操作成功';
			...
			if(data.code=='succeed'){
				...
				art.dialog({icon:'succeed',title:'操作结果',  content: content,time:3,close:function(){
					art.dialog.close();
				}});
			}else{
				art.dialog({icon:'error',title:'操作结果',  content:error,time:3});
			} 
		},'json');
	}
