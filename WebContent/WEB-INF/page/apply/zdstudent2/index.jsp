...
function ManagerStudent(){
	...
	this.load = function () {
		loadByPage('/zd_student2/list.action',{name:$("#name").val(),examYear:$("#examYear").val(),gender:$("#gender").val(),admit:$("#admit").val(),graduate:$("#graduate").val(),pageSize:10,isSetRoomNo:$('#isSetRoomNo').val()},this.showList);
	};
	...
	this.edit=function(id){
		art.dialog.open('/zd_student2/toEdit.action?id='+id,{
			title : '修改报名信息',
			width: '75%',
			height: '95%'
		});
	...
	this.delMulti =function (){
		...
		art.dialog.confirm("您确定要删除 [ "+names.join(",")+" ] 的报名信息吗？", function(){
			$.post('/zd_student2/delete.action',{ids:ids.join(',')},function(data){
	...
	this.changeStatus =function (status){
		...
		if (status==1) {//审核通过
			art.dialog.confirm("您确定要"+temp+" [ "+names.join(",")+" ] 吗？", function(){
				$.post('/zd_student2/doChangeStatus.action',{ids:ids.join(','),status:status},function(data){
				    if(data.code=='succeed'){
			...
			art.dialog.open('/zd_student2/toCheckRemark.action?status='+status+"&ids="+ids.join(',')+"&names="+names.join(','),{
				title:temp+'备注',
				width:'500px',
				height:'200px'
			});
	...
	this.initPassword =function (){
		...
		art.dialog.confirm("您确定要初始化 [ "+names.join(",")+" ] 的密码吗？", function(){
			$.post('/zd_student2/updatePassword.action',{ids:ids.join(',')},function(data){
			    if(data.code=='succeed'){
	...
	this.setInterviewDate = function(){
		...
		art.dialog.open('/zd_student2/toSetInterViewDate.action?ids='+ids.join(',')+"&names="+names.join(','),{
			title : '设置学生面试时间',
			width : 'auto',
			height : 'auto'
		});
	...
	this.setRoomNo = function(){
		...
		$("input[type='checkbox'][name='chk_student_id']:checked").each(function(){
			ids.push($(this).val());
			names.push($(this).parent().next().next().text());
			if($(this).parent().next().next().next().next().next().next().next().next().text().trim() == '未定'){
				timeNotpassNames.push($(this).parent().next().next().text());
			}
		});
		...
		art.dialog.open('/zd_student2/toSetRoomNo.action?ids='+ids.join(',')+"&names="+names.join(','),{
			title : '设置学生面试课室',
			width : 'auto',
			height : 'auto'
		});
	...
	this.showInportInterviewDate=function (){
		excelImport('&workbook=interviewDateAndBatch2', '导入面试日期和批次', '');
	};
	//下载面试时间/批次导入模版
	this.downStudentTemp=function(){
		excelExport('interviewDateAndBatch2','导入面试日期和批次模版');
	};
}
	...
	<legend class="legend_title">考生信息管理</legend>
		...
		<table  class="table_border">
			<tr class="head">
				...
				<td style="min-width: 60px;" class="thead">面试教室</td>
