...
function ManagerExam(){
	this.load = function () {
		loadByPage('/mh_exam/list.action', {examName:$("#keyword").val(), examType:1}, this.showList);
	};
	this.showList= function(list){
		...
		for ( var i = 0; i < list.length; i++) {
			...
			$('#tableBody').append(
					...
					'		<a href="/mh_exam/toPrint.action?examType=1&examId='+item.id+'" target="_blank">打印考场</a>'+
					'	</td>'+
					'</tr>'
			); 
	...
	this.create = function(id){
		if(!id)id='';
		art.dialog.open('/mh_exam/toEdit.action?examType=1&id='+id,{
	...
	this.del =function (id){
		art.dialog.confirm('确定删除吗？',function(){
			$.post('/mh_exam/delete.action',{id:id,examType:1},function(data){
				if(data=='succeed'){
					managerExam.load();
					$("#tableBody2").html('');
				}else{
	...
	this.deployRoom=function(id){
		art.dialog.confirm('您确认要将当前未分配考场的学生分配到该次考试吗？',function(){
			$.post('/mh_exam/deployRoom.action',{id:id,examType:1},function(data){
				if(data=="succeed"){
	...
	this.showExamStudent = function(id){
		...
		art.dialog.open('/mh_exam/toShowExamStudent.action?examType=1&id='+id,{
			title:examName+' 考生列表',
			width:'75%',
			height:'90%'
		});
	...
	this.print = function(examId){
		...
		if(ids.length > 0){
			art.dialog.confirm('您选中了 [ '+names.join(",")+' ]，请确认是否要打印(点击确认将进入打印预览)?',function(){
				window.open( '/mh_exam/toPrint.action?examId=' + examId);
			});
		}else{
			if(year){
