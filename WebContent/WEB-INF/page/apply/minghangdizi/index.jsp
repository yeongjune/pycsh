...
function ManagerStudent(){
	this.load = function () {
		loadByPage('/dzStudent/list.action',{year:$("#year").val(),keyword:$("#keyword").val(),pageSize:10},this.showList);
	};
	...
	this.delMulti =function (){
		...
		art.dialog.confirm("您确定要删除 [ "+names.join(",")+" ] 的报名信息吗？", function(){
			$.post('/dzStudent/delete.action',{ids:ids.join(',')},function(data){
	...
	this.view = function (id){
		art.dialog.open('/dzStudent/view.action?id='+id,{
			title : '初一新生报名明细',
			width: '75%',
			height: '95%'
		});
	...
	this.excelExport = function (){
		...
		if(ids.length > 0){
			art.dialog.confirm('您选中了 [ '+names.join(",")+' ]，是否要导出?',function(){
				excelExport('dzStudent',(year?year+'年':'')+'民航子弟学校初中新生报名信息',(year?('year='+year+'&'):'')+'ids='+ids.join(','));
			});
		}else{
			if(year){
				art.dialog.confirm('您没有选中任何学生,将导出'+year+'年民航子弟学校初中所有新生报名信息，请确认？',function(){
					excelExport('dzStudent',year+'年民航子弟学校初中所有新生报名','year='+year);
				});
			}else{
				art.dialog.confirm('您没有选择年份和任何学生，将导出所有包括以往年份的报名记录信息，请确认?',function(){
					excelExport('dzStudent','民航子弟学校初中所有新生报名(包括往年)');
				});
			}
		}
	};
...
<fieldset class="fieldset_border">
	<legend class="legend_title">初中新生报名管理</legend>
		<div class="form_search_item">
			...
			<div class="right">
					...
					关键字：<input type="text" class="piece" style="width:200px" placeholder="姓名/学校/身份证/学籍号" id="keyword"  onkeydown="if(event.keyCode==13){mngStudent.load();}" />
