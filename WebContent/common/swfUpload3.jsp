...
<div>
	...
	<a href="#" class="base_btn" id="btnUpload_<%=index%>" title="可以选择多个同时上传，已存在的将上传失败"><span>上传文件</span></a>
...
<script type="text/javascript">
	...
	function myUploadStart(){
		if(!static_select){
			art.dialog.alert('请先选择要上传的目录');
			return false;
		}else{
			swfu.addPostParam("directory",static_select); 
			return true;
		}
	}
	//上传成功后处理
	...
	(function() {
		try {
			...
			if($.trim(types) == "null"){
				types = "*.jsp;*.css;*.png;*.jpeg;*.jpg;*.bmp";
			}
			if($.trim(size) == "null"){
				size = "200 MB";
			}
			...
			var settings = {
				...
				upload_url : "/fileManager/upload.action",
