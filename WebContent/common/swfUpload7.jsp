...
<div>
	...
	<a href="#" class="base_btn" id="btnUpload_<%=index%>" style="background: #409c5a"><span style="background: #409c5a">上传文件</span></a>
...
<script type="text/javascript">
	...
	function uploadTemplateFileSuccess(file,serverData){
		...
		try {
			...
			if(code=="succeed"){
				$("#typeValue").val(result.url);
				$("#typeName").val(result.fileName);
				art.dialog.close();
			}else if(code=="false"){
				alert(result.msg);
			}else{
	...
	(function() {
		try {
			...
			var settings = {
				...
				upload_url : "/cshImage/saveFile.action?type=6",
